...
if [ "$SKIP_NGINX" -eq 1 ]; then
  ...
  # DO NOT put `nginx -t` back in front of this as a gate.
  #
  # It was here once and it was wrong. `nginx -t` opens the pid file as part of
  # its test, and on Alpine that lives under /run/nginx — a tmpfs directory
  # that is empty after every boot and is recreated by the init script's
  # start_pre, not by nginx itself. So a perfectly good config fails the test
  # with:
  #
  #     nginx: the configuration file /etc/nginx/nginx.conf syntax is ok
  #     nginx: [emerg] open() "/run/nginx/nginx.pid" failed (2: No such file...)
  #
  # — the first line saying the syntax is fine while the last calls it a
  # failure. Gating on that refuses to start an nginx that would have started
  # perfectly well, which is worse than the terse message it was meant to
  # improve on.
  #
  # The readable diagnosis is still worth having, so it happens AFTER a real
  # failure instead: let rc-service try, and only reach for nginx -t to explain
  # why it did not work. Act first, diagnose on failure.
  say "Starting nginx…"
  if rc-service nginx start; then
    ...
    if command -v nginx >/dev/null 2>&1; then
      ...
      # Said plainly, because this test lies in one specific way and the lie
      # is convincing: it reports a pid-file error for a config that is fine.
      say ""
      say "  Read that carefully. If one line says the syntax is ok and a"
      say "  later one complains about /run/nginx or a pid file, the config"
      say "  is NOT the problem — /run is emptied at every boot and the"
      say "  runtime directory has not been recreated. Usually:"
      say "        mkdir -p /run/nginx && chown nginx:nginx /run/nginx"
